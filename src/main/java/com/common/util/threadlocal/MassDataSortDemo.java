...
/**
 * 题目：文件中保存了10亿条数据，数据类型为整数，取值范围[1, 10000]，现需要对这10亿条数据进行排序，
 *      请使用Java编程实现。
 * 思路：1，将大数据的文件进行拆分指定个数的文件个数，countFile
 *      2，对内一个countfile中的文件中的数据进行排序
 *      3，合并使用LinkedList中的Sort方法
 *
 * @author: zhoucg
 * @date: 2019-08-28
 */
public class MassDataSortDemo {

    //文件中数据的总个数
    private static final int TOTAL_NUMBER_COUNT = 100000000;
    ...
    public static void main(String[] args) throws IOException {
        ...
        String sqlitFilePath = "D:\\filedir\\split";
        ...
        // =====================测试模拟数据=================================
        WriteData(filePath,TOTAL_NUMBER_COUNT);
        // =====================结束=================================

        //1,将大数据文件切分到另外的十个小文件中
    ...
    public static void WriteData(String path,int CountNumbers) throws IOException {
        ...
        for (int i = 0; i < CountNumbers; i++) {
            fw.write(new Random().nextInt(10000) + "\r\n");
        }
        fw.close();
        fs.close();
